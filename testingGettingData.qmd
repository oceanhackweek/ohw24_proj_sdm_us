...
```{r}
 ...
 data = GET(paste0("https://erddap.bio-oracle.org/erddap/griddap/",
...
```{r}
bin <- content(res, "raw")
writeBin(bin, "data1.csv")

# Read as csv
dat = read.csv("data1.csv", header = TRUE, dec = ",")
```
...
dat %>% 
  ...
  mutate(dataset = as.numeric(tas_ltmin)) %>% 
