...
load_layers <- (layers[layers$name %in% c("pH", "Bathymetry (mean)", "Chlorophyll A (mean)", "Dissolved oxygen concentration [02]", "Nitrate", "Salinity", "Sea surface temperature (mean)") & 
...
vars <- layers$layers_code %in% c("BO_ph","BO_bathymean","BO_chlomean","BO_dissox","BO_nitrate","BO_salinity","BO_sstmean")
vars <- layers$name
...
```{r}
layers %>% 
  filter(layer_code %in% c("BO_ph","BO_bathymean","BO_chlomean","BO_dissox","BO_nitrate","BO_salinity","BO_sstmean"))
```
```{r}
layers %>% 
